...
class ControlsMenu extends ui.OptionsState.Page
{
	...
	public function new()
	{
		...
		if (FlxG.gamepads.numActiveGamepads > 0)
		{
			...
			item = deviceList.createItem(0,0,"Keyboard", Bold, function(?v) selectDevice.bind(Keys));
			item.x = FlxG.width / 2 - item.width - 30;
			item.y = (devicesBg.height - item.height) / 2;

			item = deviceList.createItem(0,0,"Gamepad", Bold, function(?v) selectDevice.bind(Gamepad(FlxG.gamepads.firstActive.id)));
