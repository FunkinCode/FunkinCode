...
class MenuTypedList<T:MenuItem> extends FlxTypedGroup<T>
{
	...
	public function new (navControls:NavControls = Vertical, ?wrapMode:WrapMode, ?defaultSelectedIndex:Int = 0)
	{
	...
	public function resetItem(oldName:String, newName:String, ?callback:(?value:Dynamic)->Void):T
	{
	...
	function navAxis(index:Int, size:Int, prev:Bool, next:Bool, allowWrap:Bool):Int
	{
		...
		} while(prevItem(index).ignoreMePlz) ;
	...
	public function accept(?value:Dynamic)
	{
...
{
	public var callback:(?value:Dynamic)->Void;
	public var isFloatMovement:Bool = false;
	public var justPress:Bool = false;
	public var ignoreMePlz:Bool = false;
	...
	function setData(name:String, ?callback:(?value:Dynamic)->Void)
	{
		this.name = name;
	 ...
	 */
	public function setItem(name:String, ?callback:(?value:Dynamic)->Void)
	{
		setData(name, callback);
